...
First executing the follow command:

    docker-compose up -d tracing-server grafana-server prometheus-server

Every microservice is a Spring Boot application and can be started locally using IDE or `../mvnw spring-boot:run` command or `..\mvnw.cmd spring-boot:run` command on Windows OS.
